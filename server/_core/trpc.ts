...
import { protocolEndpoints } from "../../drizzle/schema";
...
function ruleIdsFromMutationInput(path: string, input: unknown) {
  ...
  return path === "rules.deleteBatch"
    ? positiveRuleIds(Array.isArray(value.ids) ? value.ids : [])
    : positiveRuleIds([value.id]);
}
...
function endpointOwnsManagedTrafficBridgeRule(endpoint: { forwardRuleId: unknown; configJson: unknown }, ruleIds: Set<number>) {
  ...
  if (!Number.isInteger(linkedRuleId) || linkedRuleId <= 0 || !ruleIds.has(linkedRuleId)) return false;
...
async function assertUserRuleMutationAllowed(path: string, input: unknown) {
...
const requireUser = t.middleware(async opts => {
  ...
  if (ctx.user.role !== "admin" && USER_MUTABLE_RULE_PATHS.has(opts.path)) {
    await assertUserRuleMutationAllowed(opts.path, await opts.getRawInput());
  }
