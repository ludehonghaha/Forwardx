...
if (
  ...
  throw new Error(
    "usage: materialize-dual-existing-hy2-gray-local.ts <outside-repo-output-dir> <verified-private-endpoint-file> <verified-existing-hy2-discovery-file> <hy2-auth-file> <hy2-obfs-password-file> <mieru-username-file> <mieru-password-file>",
  );
}
...
const draft = dualMultipathDraftSchema.parse({
  version: 5,
  state: "draft",
  name: "NoBrand Dual reuse existing ForwardX HY2 Gray",
  ...infrastructure,
  line: {
    ...infrastructure.line,
    activationThresholdMbps: 120,
    activationAfterBytes: 8 * 1024 * 1024,
    activationWindow: "1s",
  },
  legs: [
...
writePrivateJson("materialization-metadata.json", {
  purpose: "local Gray reuse-existing-hy2 validation only",
...
console.log(`Materialized secret-bearing existing-HY2 Gray configs outside the repository: ${outputDir}`);
console.log("Secrets were not printed; server Gray contains only the loopback multipath inbound; readyForRuntime remains false.");
