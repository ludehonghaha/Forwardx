...
FORWARDX_IMAGE=ghcr.io/ludehonghaha/forwardx:latest
...
services:
  forwardx:
    image: ${FORWARDX_IMAGE:-ghcr.io/ludehonghaha/forwardx:latest}
...
docker image ls --no-trunc --format '{{.Repository}} {{.Tag}} {{.ID}}' ghcr.io/ludehonghaha/forwardx \
  | awk -v current="$CURRENT_IMAGE_ID" '$1 == "ghcr.io/ludehonghaha/forwardx" && $2 != "<none>" && $3 != current { print $1 ":" $2 }' \
...
```bash
curl -fsSL https://raw.githubusercontent.com/ludehonghaha/Forwardx/main/scripts/install-panel-local.sh \
...
mkdir -p "$APP_DIR"
curl -fL "https://github.com/ludehonghaha/Forwardx/releases/download/${VERSION}/forwardx-panel-${VERSION}.tar.gz" -o /tmp/forwardx-panel.tar.gz
